  ...
  const handleBulkImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        await api.post('/teacher/questions/bulk', { questions: json });
        alert('Bulk import successful');
        fetchQuestions();
      } catch {
        alert('Invalid JSON file format or server error');
      }
    };
    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4 fade-in">
        ...
        <div className="d-flex gap-2">
          <label className="btn btn-outline-primary m-0 d-flex align-items-center">
            <Upload size={18} className="me-2" /> Bulk Import JSON
            <input type="file" accept=".json" hidden onChange={handleBulkImport} />
          </label>
